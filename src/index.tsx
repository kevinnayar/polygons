...
class App extends React.Component<{}, TypeAppState> {
  ...
  handleClick(page: number, max: number, forward: boolean) {
  ...
  handleKeyDown = (event: KeyboardEvent) => {
    // right
    if (event.keyCode === 39) {
      this.handleClick(this.state.page, this.NUM_PAGES, true);
    }
    // left
    if (event.keyCode === 37) {
      this.handleClick(this.state.page, this.NUM_PAGES, false);
    }
  };
            ...
            <Wrapper className="polygons" height={this.HEIGHT}>
              ...
              <Nav
                ...
                onNavClick={(forward: boolean) =>
                  this.handleClick(this.state.page, this.NUM_PAGES, forward)
                }
              />
            </Wrapper>
            <Photos folder={pageData.id} max={this.NUM_IMAGES} />
          </Wrapper>
        )}
      </Wrapper>
    );
